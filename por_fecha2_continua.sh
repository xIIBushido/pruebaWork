...
if [ -z "$FECHA_ACTUALIZACION" ] || [ "$FECHA_ACTUALIZACION" == "NA" ]
  then 
    FECHA_ACTUALIZA=$(date +%Y-%m-%d --date="-$DIAS_POR_FECHA day")
    FILTROS=$(echo $FILTROS | sed -e ''s/P_FCH_INI/$FECHA_ACTUALIZA/g'')
  else
    FECHA_ACTUALIZA=$FECHA_ACTUALIZACION
    FILTROS2=$(echo $FILTROS2 | sed -e "s/P_FCH_INI/$FECHA_ACTUALIZA/g")
fi;

echo "FECHA_ACTUALIZA: $FECHA_ACTUALIZA" 

if [ -z "$FILTROS"  ] || [ "$FILTROS" = "na" ] || [ "$FILTROS" = "NA" ]; then FILTROS=""; fi; echo "filtros $FILTROS";
if [ -z "$FILTROS2"  ] || [ "$FILTROS2" = "na" ] || [ "$FILTROS2" = "NA" ]; then FILTROS2=""; fi; echo "filtros $FILTROS2";
...
PATH_PARQUET_FILES="/mnt/data013/raw_files/rdata"
#####################################
############Auditoria
#####################################
...
V_T_Terminal=$($ODB -u ''$V_BIT_N_ORG_USR'' -p ''$V_BIT_X_ORG_USR'' -d $BITACORA_DSN -x "UPDATE $V_BIT_OWNER.TBL_HCH_RESULTADOS_LObd SET RESULTADO='EJECUTANDO' WHERE PID='$PID_PADRE' and INC=$INC" 2>&1)
...
#function principal {
    #FECHA_INICIO  
    UPDATE_ESTATUS=$($ODB -u ''$V_BIT_N_ORG_USR'' -p ''$V_BIT_X_ORG_USR'' -d $BITACORA_DSN -x  "UPDATE $V_BIT_OWNER.TBL_HCH_RESULTADOS_LObd SET FECHA_INICIO='$FECHA_INICIO' WHERE PID='$PID_PADRE' and INC=$INC" 2>&1)
    ...
    then 
        ...
        V_T_Terminal=$($ODB -u ''$V_BIT_N_ORG_USR'' -p ''$V_BIT_X_ORG_USR'' -d $BITACORA_DSN -x  "UPDATE $V_BIT_OWNER.TBL_HCH_RESULTADOS_LObd SET  FECHA_FIN='$FECHA_FIN',RESULTADO='$RESULTADO' WHERE PID='$PID_PADRE' and INC=$INC" 2>&1)
        echo FECHA_FIN $FECHA_FIN
        UPDATE_ESTATUS=$($ODB -u ''$V_BIT_N_ORG_USR'' -p ''$V_BIT_X_ORG_USR'' -d $BITACORA_DSN -x  "UPDATE $V_BIT_OWNER.TBL_HCH_RESULTADOS_LObd SET TODOS_ORIGEN=$TODOS_ORIGEN, PARCIAL_ORIGEN=$PARCIAL_ORIGEN TODOS_DESTINO_A=$TODOS_DESTINO_A, TODOS_DESTINO_AA=$TODOS_DESTINO_AA, TODOS_STG=$TODOS_STG, TODOS_DESTINO=$TODOS_DESTINO, PARCIAL_DESTINO=$PARCIAL_DESTINO WHERE PID='$PID_PADRE' and INC=$INC" 2>&1)
        exit 1
    else
        ...
        timeout --signal=9 $(($TIEMPO_EJEC + 60)) $ODB -u ''$USUARIO'' -p ''$CONTRASENA'' -d $CONEXION -soe -timeout $TIEMPO_EJEC -pcn -e "sql=[select $HINTSS $CAMPOS_ORIGEN from $ESQUEMA_ORIGEN.$TABLA_ORIGEN TBL where $CAMPO_BASE_ORA >= TO_DATE ('$FECHA_ACTUALIZA','YYYY-MM-DD') $FILTROS]:tgt=$ODB_COPY_OPC" 2>>$RUTA_LOG | python3 $PARQUE_BASE/parque_loader_v2.py "$ESQUEMA_ORIGEN"."$TABLA_ORIGEN$PYTHON_CHUNK" "$FECHA_PROCESO" "$TABLA_DDL" >> "$RUTA_LOG"      
          ...
          echo "$ODB -u '$USUARIO' -p 'XXXXXXXXXXXXX' -d $CONEXION -soe -timeout $TIEMPO_EJEC -pcn -e sql=[select $HINTSS $CAMPOS_ORIGEN from $ESQUEMA_ORIGEN.$TABLA_ORIGEN TBL where TO_CHAR($CAMPO_BASE_ORA,'yyyy-MM-dd HH24') >= '$FECHA_ACTUALIZA' $FILTROS2]:tgt=$ODB_COPY_OPC" 
          timeout --signal=9 $(($TIEMPO_EJEC + 60)) $ODB -u ''$USUARIO'' -p ''$CONTRASENA'' -d $CONEXION -soe -timeout $TIEMPO_EJEC -pcn -e "sql=[select $HINTSS $CAMPOS_ORIGEN from $ESQUEMA_ORIGEN.$TABLA_ORIGEN TBL where TO_CHAR($CAMPO_BASE_ORA,'yyyy-MM-dd HH24') >= '$FECHA_ACTUALIZA' $FILTROS2]:tgt=$ODB_COPY_OPC" 2>>$RUTA_LOG | python3 $PARQUE_BASE/parque_loader_v2.py "$ESQUEMA_ORIGEN"."$TABLA_ORIGEN$PYTHON_CHUNK" "$FECHA_PROCESO" "$TABLA_DDL" >> "$RUTA_LOG"
        fi
